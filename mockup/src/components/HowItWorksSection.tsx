...
const steps = [
  {
    number: '1',
    title: '제품 사진 업로드',
    description: '스마트폰으로 찍은 사진이면 충분합니다. 바닥컷, 행거컷 모두 OK.',
    icon: Upload,
  },
  {
    number: '2',
  ...
  },
  {
    number: '4',
    title: '다운로드 & 적용',
    description: '고해상도 결과물을 바로 다운로드해서 쇼핑몰에 적용하세요.',
    icon: Download,
  },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1100px] mx-auto">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative bg-white rounded-[20px] p-8 shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:-translate-y-2 hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] transition-all duration-300 ${
              ...
              >
                {/* Step Icon */}
                <div className="mb-5">
                  <Icon size={48} className="text-[#9E9E9E] hover:text-[#1A1A1A] transition-colors" strokeWidth={1.5} />
                </div>
                ...
                {/* Step Description */}
                <p className="text-[15px] text-[#6B6B6B] leading-[1.6]">{step.description}</p>
